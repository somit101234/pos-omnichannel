...
@Injectable()
export class BeFoodAdapter implements IPlatformAdapter {
  private readonly platform: 'BEOFORD' = 'BEOFORD';
  private readonly platformFeeRate = 0.18; // Default 18%
  ...
  parse(file: Buffer): OrderItem[] {
    ...
    for (const row of dataRows) {
      const rowObj = this.rowToObj(header, row);
